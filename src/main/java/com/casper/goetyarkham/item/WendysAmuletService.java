...
/**
 * Owns the single +1 weakness slot and the Abandoned and Alone bound to an
 * equipped Wendy's Amulet. Persistent state distinguishes a real equip
 * transition from capability loading, login, dimension synchronization, and
 * player cloning.
 */
public final class WendysAmuletService {
    public static final UUID WEAKNESS_SLOT_MODIFIER_ID = UUID.fromString(
            "c2b6f204-9e3a-4a1a-8d7f-2a6b9c4e5f10");
    public static final String WEAKNESS_SLOT_MODIFIER_NAME =
            "goetyarkham:wendys_amulet";

    private static final String STATE_KEY =
            "GoetyArkhamWendysAmulet";
    private static final String ACTIVE_KEY = "Active";
    private static final String BINDING_KEY = "AbandonedAndAloneBinding";
    private static final String SLOT_KEY = "AbandonedAndAloneSlot";

    private WendysAmuletService() {
    }
    ...
    private static boolean equipTransition(
        ...
        if (weaknesses == null) {
            GoetyArkham.LOGGER.error(
                    "[WendysAmulet] Missing weakness handler for player {}",
                    player.getUUID());
            clearState(player);
        ...
        if (slot < 0) {
            GoetyArkham.LOGGER.error(
                    "[WendysAmulet] No empty weakness slot after applying modifier for player {}",
                    player.getUUID());
            clearState(player);
            removeWeaknessSlot(inventory);
            return false;
        ...
        ItemStack weakness = AbandonedAndAloneItem.createBound(
    ...
    public static int equippedCount(ServerPlayer player) {
        return CuriosApi.getCuriosInventory(player).resolve()
                .map(WendysAmuletService::equippedCount)
                .orElse(0);
    }
        ...
        if (keptSlot < 0) {
            ...
            if (keptSlot >= 0) {
                weaknesses.getStacks().setStackInSlot(
                        keptSlot,
                        AbandonedAndAloneItem.createBound(player.getUUID(), binding));
            }
        }
        if (keptSlot >= 0) {
    ...
    /**
     * Keeps at most one exact binding and deletes every other Wendy's
     * Amulet-owned weakness for this player. Passing null deletes all owned
     * weaknesses.
     */
    private static int deleteOwnedWeaknesses(
        ...
        for (int slot = 0; slot < stacks.getSlots(); slot++) {
            ...
            if (!AbandonedAndAloneItem.isAmuletBound(stack, player.getUUID())) {
                continue;
            }
            if (bindingToKeep != null
                    && keptSlot < 0
                    && AbandonedAndAloneItem.isAmuletBound(
    ...
    private static int equippedCount(ICuriosItemHandler inventory) {
        return inventory.getStacksHandler(CurioSlotIds.CHARM)
                .map(handler -> {
                    int count = 0;

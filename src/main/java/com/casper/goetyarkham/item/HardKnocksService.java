...
import com.casper.goetyarkham.client.ClientHardKnocksBonus;
...
/**
 * Owns Hard Knocks' participation in the shared {@link
 ...
 * HardKnocksBonusProvider} once (on first class load) and, on every
 ...
 */
public final class HardKnocksService {
    private static final List<String> WORN_SLOTS = List.of(CurioSlotIds.ASSET);

    static {
        ResourceSlotProviderRegistry.register(HardKnocksBonusProvider.INSTANCE);
    }

    private HardKnocksService() {
    }
    ...
    public static boolean isWearing(LivingEntity entity) {
        return DynamicCurioSlotContributionService.isWearing(
                entity, ModItems.HARD_KNOCKS.get(), WORN_SLOTS);
    }
     ...
     * resource} contribution, for {@code HardKnocksItem}'s Shift tooltip.
    ...
    public static ResourceSlotBonusSnapshot currentClientBonus() {
        ResourceSlotBonusSnapshot result = DistExecutor.unsafeCallWhenOn(
                Dist.CLIENT, () -> ClientHardKnocksBonus::compute);

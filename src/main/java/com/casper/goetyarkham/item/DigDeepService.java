...
import com.casper.goetyarkham.client.ClientDigDeepBonus;
...
/**
 * Owns Dig Deep's participation in the shared {@link CurioSlotIds#RESOURCE}
 * slot pool, worn in {@link CurioSlotIds#ASSET}. Mirrors {@link
 * PhysicalTrainingService} exactly: registers {@link DigDeepBonusProvider}
 * once (on first class load) and, on every observed equip/unequip
 * transition, asks {@link ResourceSlotService} to recompute the shared
 * slot's capacity from every currently registered provider's equip state -
 * the same generic mechanism the Encyclopedia and Physical Training already
 * use, so all of them coexist without any one needing to know about the
 * others.
 */
public final class DigDeepService {
    private static final List<String> WORN_SLOTS = List.of(CurioSlotIds.ASSET);

    static {
        ResourceSlotProviderRegistry.register(DigDeepBonusProvider.INSTANCE);
    }

    private DigDeepService() {
    }
    ...
    public static boolean isWearing(LivingEntity entity) {
        return DynamicCurioSlotContributionService.isWearing(
                entity, ModItems.DIG_DEEP.get(), WORN_SLOTS);
    }
     ...
     * resource} contribution, for {@code DigDeepItem}'s Shift tooltip.
     * Mirrors {@link PhysicalTrainingService#currentClientBonus()}.
     */
    public static ResourceSlotBonusSnapshot currentClientBonus() {
        ResourceSlotBonusSnapshot result = DistExecutor.unsafeCallWhenOn(
                Dist.CLIENT, () -> ClientDigDeepBonus::compute);
